...
rootwait parameter is required when booting from emmc).
...
The bootable sdcard.img image file is created by the genimage utility.  Two
filesystem images are created in addition to the rootfs image created by
Buildroot:
...
image. The first is offset by 64K from the start of the device (to leave room
for the U-Boot environment) and appears in the partition table.  The second copy
is offet 32M from the first and is not listed in the partition table (this is
...
* Example application that interacts with the systemd watchdog and triggers rauc
  status set-good once everything is up and running.
* Boot-time configuration of watchdog timer (RPi watchdog code was pulled from
  U-Boot in 2019).
